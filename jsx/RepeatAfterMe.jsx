...
(function () {
  ...
  var prefs;
  prefs = new Prefs(
    Folder.userData + "/JBD/" + scriptTitle + ".json",
    scriptVersion,
  );
  prefs.load(defaults);
  ...
  // setup a template layer for preview
  var templateLayer;
  ...
  function dialog() {
    ...
    var stRows = gRows.add("statictext", undefined, "Rows:", {
      name: "stRows",
    });
    ...
    var rows = gRows.add(
      'edittext {justify: "center", properties: {name: "rows"}}',
    );
    rows.text = "";
    ...
    var stCols = gCols.add("statictext", undefined, "Columns:", {
      name: "stCols",
    });
    ...
    var cols = gCols.add(
      'edittext {justify: "center", properties: {name: "cols"}}',
    );
    cols.text = "";
    ...
    var divider1 = pLayout.add("panel", undefined, undefined, {
      name: "divider1",
    });
    ...
    // Panel - Bounds
    var pBounds = gBoundsInfo.add("panel", undefined, "Bounds", {
      name: "pBounds",
    });
    ...
    var visible = gBounds.add("radiobutton", undefined, "Visible", {
      name: "visible",
    });
    ...
    // Panel - Info
    var pInfo = gBoundsInfo.add("panel", undefined, "Repeat Info", {
      name: "pInfo",
    });
    ...
    gCopies.add("statictext", undefined, "Total Copies:", {
      name: "stCopies",
    });

    var copies = gCopies.add("statictext", undefined, "12345", {
      name: "copies",
    });
    ...
    gWidth.add("statictext", undefined, "Width:", {
      name: "stWidth",
    });
    ...
    gHeight.add("statictext", undefined, "Height:", {
      name: "stHeight",
    });
    ...
    function getCurrentDialogSettings() {
      ...
      return {
        ...
        center: center.value,
        rows: parseInt(rows.text) || 1,
        rowGutter: UnitValue(rowGutter.text).toString(),
        cols: parseInt(cols.text) || 1,
    ...
    function calculateTranslationDeltas() {
      ...
      var _rows = parseInt(rows.text) || 1;
      var _rowGutter = UnitValue(rowGutter.text).as("pt");
      var _cols = parseInt(cols.text) || 1;
    ...
    function update() {
      try {
        ...
        copies.text = (
          (parseInt(rows.text) || 1) * (parseInt(cols.text) || 1)
        ).toString();
    ...
    function processChanges() {
      ...
      if (fill.value) {
        ...
        var availableWidth, availableHeight, calcRows, calcCols;
    ...
    var integerInputs = [rows, cols]; // int inputs
    for (var i = 0; i < integerInputs.length; i++) {
      ...
      integerInputs[i].addEventListener(
        "keydown",
        editTextArrowAdjustmentsRowCol,
      );
    }

    // validate `edittext` float inputs on before processing any template changes
    var floatInputs = [rowGutter, colGutter, padding];
    for (var j = 0; j < floatInputs.length; j++) {
      // add validation method
      floatInputs[j].validate = function () {
        var n;
        logger.log("validating:", this.properties.name, "(" + this.text + ")");
      ...
      // add arrow key listener
      floatInputs[j].addEventListener(
        "keydown",
        editTextArrowAdjustmentsGutter,
      );
    }
    ...
    for (var n = 0; n < textInputs.length; n++) {
      textInputs[n].onChange = function () {
        if (Object.prototype.hasOwnProperty.call(this, "validate")) {
          this.validate();
        }
    ...
    for (var p = 0; p < boundsRadioButtons.length; p++) {
      boundsRadioButtons[p].onClick = function () {
        logger.log("changed:", this.properties.name, "(" + this.text + ")");
