...
1.2.0 2024-10-21 added ordering by artboard placement
...
(function () {
  ...
  if (sel.length < 2) {
    alert("Not enough objects to selected. Select at least two objects.");
    return;
  }
  ...
  function calculateMoveMatrix(sourceBounds, targetBounds, direction, gutter) {
    ...
    if (direction == "Vertical" && sourceInfo.bottom != targetInfo.top) {
      ...
      sourceInfo.right != targetInfo.left
    ) {
      return app.getTranslationMatrix(
  ...
  function settingsWin() {
    ...
    var rbStackingOrder = gOrderBy.add(
      "radiobutton",
      undefined,
      "Stacking Order",
    );
    rbStackingOrder.value = true;
    gOrderBy.add("radiobutton", undefined, "Horizontal Placement");
    gOrderBy.add("radiobutton", undefined, "Vertical Placement");
    ...
    pGutter.add("statictext", undefined, "Size:");
    var gutter = pGutter.add('edittext {justify: "center"}');
    gutter.preferredSize.width = 100;
    gutter.text = "0 pt";
    ...
    gWindowButtons.add("button", undefined, "OK");
    ...
    if (win.show() == 1) {
      return {
        ...
        gutter: parseNumberInput(gutter.text).as("px"),
      };
    } else {
      return;
    }
