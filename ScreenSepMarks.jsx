...
function writeInfo(location, alignment, spots, file, date, time) {
    ...
    if (spots && spotColors.length > 1) {
        for (var i = 1; i < spotColors.length; i++) {
            var swatchName = spotColors[i].name
                .replace(/ /g, "_")
                .toUpperCase();
    ...
    settings.onChange = function () {
        if (settings.selection) {
            ...
            if (settingsValue) {
                ...
                strokeUnit.selection = indexOf(
                    arrUnits,
                    loadedSettings.strokeUnit
                );
                insetUnit.selection = indexOf(
                    arrUnits,
                    loadedSettings.insetUnit
                );
                color.selection = color.find(loadedSettings.color)
                    ? color.find(loadedSettings.color)
                    : color.find("[Registration]");
                position.selection = indexOf(
                    arrPosition,
                    loadedSettings.position
                );
                alignment.selection = indexOf(
                    arrAlignment,
                    loadedSettings.alignment
                );
            }
        }
    };
