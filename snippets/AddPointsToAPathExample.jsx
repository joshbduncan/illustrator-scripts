...
De Casteljau's algorithm which can be used to calculate a point any percentage between
...
(function () {
  ...
  if (app.documents.length > 0) {
    ...
    if (doc.selection.length == 1) {
      ...
      if (settings) {
        ...
        if (
          anchors.length == parentPath.pathPoints.length &&
          parentPath.closed
        ) {
        ...
        // show the results
        var hl, left, top;
        for (var j = 0; j < hlPathPoints.length; j++) {
          // add red highlight points at each calculated anchor
          if (settings.hlPoints) {
            left = hlPathPoints[j].anchor[0] - hlSize / 2;
            top = hlPathPoints[j].anchor[1] + hlSize / 2;
          ...
          if (settings.drawPath) {
            ...
            p.anchor = hlPathPoints[j].anchor;
            p.leftDirection = hlPathPoints[j].leftDirection;
            p.rightDirection = hlPathPoints[j].rightDirection;
          }
        }
      }
    } else {
      ...
      },
      {
        ...
        rightDirection:
          controlPoints.length > 0
            ...
            : points[0],
        pointType:
          start.pointType == PointType.CORNER &&
          end.pointType == PointType.CORNER
            ? PointType.CORNER
            : PointType.SMOOTH,
      },
      {
  ...
  function setupHighlights() {
    hlGroup = doc.activeLayer.groupItems.add();
  ...
  function settingsWin() {
    ...
    var cbHlPoints = gOptions.add("checkbox", undefined, "Highlight Points?");
    cbHlPoints.value = true;
    var cbDrawNewPath = gOptions.add("checkbox", undefined, "Draw Path?");
    cbDrawNewPath.value = true;

    // panel - single points
    var pSinglePoint = w.add(
      "panel",
      undefined,
      "Place Single Point At Position",
    );
